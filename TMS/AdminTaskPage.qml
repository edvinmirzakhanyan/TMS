...
Page {
    ...
    onFocusChanged: root.showTasksList();
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Button {
                ...
                onClicked: {
                    ...
                    performerInfoPage.visible = false;
                }
            }
        }
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    Button {
                        ...
                        anchors.leftMargin: 20
                        background: Rectangle {
                            id: performerButtonBackground
                            width: parent.width
                            height: parent.height
                            color: performerButton.hovered ? "lightgray" : "white"
                            border.color: "darkmagenta"
                            border.width: 2
                            radius: 20
                        }
                        ...
                        onClicked: {
                            user.getPerformerInfo(tasksInfo.get(index).taskNameText);
                        }
                    }
                    ...
                    Button {
                        ...
                        height: projectName.height
                        width: status.width * 0.7
                        anchors.left: performerButton.right
                        anchors.leftMargin: 20
                        background: Rectangle {
                        ...
                        onClicked: {
                            if (status.text !== "Progressing") {
                            user.changeTaskStatus(tasksInfo.get(index).taskNameText, "Progressing");
                            status.color = "orange";
        ...
        function showPerformerInfo(line) {
            ...
            performerInfoPage.visible = true;
        }

        function showTasksList(lineArr = user.getProjectDevelopers()) {
            let arr;
            for (let i = 0; i < lineArr.length; ++i) {
                arr = user.split(lineArr[i]);
                tasksInfo.append( { projectNameText: arr[1], taskNameText: arr[3], statusText: arr[4],} );
            }
        }
    }
    ...
    Connections {
        target: user
        onCatchPerformerInfo: {
            root.showPerformerInfo(type);
        }
    }
}
